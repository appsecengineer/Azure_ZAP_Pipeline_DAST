...
class RoboBurp2(object):
    ...
    def start_burpsuite(self, burp_path, user_config=None,project_config=None):
        ...
        if user_config and project_config:
            cmd = "java -jar {0} --user-config-file={1} --config-file={2}".format(burp_path,user_config,project_config)
        elif user_config:
    ...
    def start_headless_burpsuite(self, burp_path, user_config=None,project_config=None):
        '''
        ...
        '''
        if user_config and project_config:
            cmd = "java -Djava.awt.headless=true -jar {0} --user-config-file={1} --config-file={2}".format(burp_path,user_config,project_config)
        elif user_config:
    ...
    def make_status_request(self, scan_id):
        ...
        '''
        url = "{}scan/{}".format(self.api_url, scan_id)
    ...
    def get_burp_scan_status_for_id(self, scan_id, status_interval = 10):
        ...
        '''
        return self.make_status_request(scan_id)

    def get_burp_scan_results_for_id(self, scan_id, filename = "burp_scan_results.json"):
        '''
        Get results of Burp Scan by id and write to file
        :param filename: provide a custom file path (absolute). must end with json
        :return:
        '''
        scan_results = self.make_status_request(scan_id)
